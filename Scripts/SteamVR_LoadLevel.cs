...
namespace Valve.VR
{
    public class SteamVR_LoadLevel : MonoBehaviour
    {
        ...
        IEnumerator LoadLevel()
        {
            ...
            {
                MelonLogger.Msg("[HPVR] Launching external application...");
                var applications = OpenVR.Applications;
                if (applications == null)
                {
                    MelonLogger.Msg("[HPVR] Failed to get OpenVR.Applications interface!");
                }
                else
                {
                    var workingDirectory = Directory.GetCurrentDirectory();
                    var fullPath = Path.Combine(workingDirectory, internalProcessPath);
                    MelonLogger.Msg("[HPVR] LaunchingInternalProcess");
                    MelonLogger.Msg("[HPVR] ExternalAppPath = " + internalProcessPath);
                    MelonLogger.Msg("[HPVR] FullPath = " + fullPath);
                    MelonLogger.Msg("[HPVR] ExternalAppArgs = " + internalProcessArgs);
                    MelonLogger.Msg("[HPVR] WorkingDirectory = " + workingDirectory);
                    var error = applications.LaunchInternalProcess(fullPath, internalProcessArgs, workingDirectory);
                    MelonLogger.Msg("[HPVR] LaunchInternalProcessError: " + error);
#if UNITY_EDITOR
