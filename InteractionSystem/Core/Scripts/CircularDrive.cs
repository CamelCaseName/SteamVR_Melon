...
namespace Valve.VR.InteractionSystem
{
    ...
    public class CircularDrive : MonoBehaviour
	{
		...
		///<summary>A LinearMapping component to drive, if not specified one will be dynamically added to this GameObject</summary>
		...
		///<summary>If limited, set whether drive will freeze its angle when the min angle is reached</summary>
		public bool freezeOnMin = false;
		///<summary>If limited, event invoked when minAngle is reached</summary>
		public UnityEvent onMinAngle;

		///<summary>If limited is true, the specifies the upper limit, otherwise value is unused</summary>
		public float maxAngle = 45.0f;
		///<summary>If limited, set whether drive will freeze its angle when the max angle is reached</summary>
		public bool freezeOnMax = false;
		///<summary>If limited, event invoked when maxAngle is reached</summary>
		public UnityEvent onMaxAngle;

		///<summary>If limited is true, this forces the starting angle to be startAngle, clamped to [minAngle, maxAngle]</summary>
		public bool forceStart = false;
		///<summary>If limited is true and forceStart is true, the starting angle will be this, clamped to [minAngle, maxAngle]</summary>
		public float startAngle = 0.0f;

		///<summary>If true, the transform of the GameObject this component is on will be rotated accordingly</summary>
		public bool rotateGameObject = true;

		///<summary>If true, the path of the Hand (red) and the projected value (green) will be drawn</summary>
		public bool debugPath = false;
		///<summary>If debugPath is true, this is the maximum number of GameObjects to create to draw the path</summary>
		public int dbgPathLimit = 50;

		///<summary>If not null, the TextMesh will display the linear value and the angular value of this circular drive</summary>
		public TextMesh debugText = null;

		///<summary>The output angle value of the drive in degrees, unlimited will increase or decrease without bound, take the 360 modulus to find number of rotations</summary>
		...
		private void OnHandHoverEnd( Hand hand )
		{
			...
			if ( driving && hand )
			{
				...
				MelonLoader.MelonCoroutines.Start( HapticPulses( hand, 1.0f, 10 ) );
			}
		...
		private Vector3 ComputeToTransformProjected( Transform xForm )
		{
			...
			{
				MelonLoader.MelonLogger.Msg("[HPVR Interaction] The collider needs to be a minimum distance away from the CircularDrive GameObject {0}", gameObject.ToString() );
				Debug.Assert( false, string.Format("[HPVR Interaction] The collider needs to be a minimum distance away from the CircularDrive GameObject {0}", gameObject.ToString() ) );
			}
				...
				if ( absAngleDelta > 0.0f )
				{
					if ( frozen )
					{
						...
						if ( frozenSqDist > frozenSqDistanceMinMaxThreshold.x )
						{
							...
							if ( magnitude > 0 )
							{
								MelonLoader.MelonCoroutines.Start( HapticPulses( hand, magnitude, 10 ) );
							}
							else
							{
								MelonLoader.MelonCoroutines.Start( HapticPulses( hand, 0.5f, 10 ) );
							}
