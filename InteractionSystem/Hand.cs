...
namespace SteamVR_Melon.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        public UnityEngine.XR.InputDeviceRole handType;
        ...
        public void AttachObject(GameObject objectToAttach, GrabTypes grabbedWithType, AttachmentFlags flags = defaultAttachmentFlags, Transform attachmentOffset = null)
        {
            ...
            if (attachedObject.HasAttachFlag(AttachmentFlags.SnapOnAttach))
            {
                ...
                if (attachedObject.interactable != null /*&& attachedObject.interactable.skeletonPoser != null*/ && HasSkeleton())
                {
                    ...
                    //objectToAttach.transform.position = transform.TransformPoint(pose.position);
                    //objectToAttach.transform.rotation = transform.rotation * pose.rotation;
            ...
            {
                //Todo
                if (attachedObject.interactable != null && HasSkeleton())
                //if (attachedObject.interactable != null && attachedObject.interactable.skeletonPoser != null && HasSkeleton())
                {
                    attachedObject.initialPositionalOffset = attachedObject.handAttachmentPointTransform.InverseTransformPoint(objectToAttach.transform.position);
                    attachedObject.initialRotationalOffset = Quaternion.Inverse(attachedObject.handAttachmentPointTransform.rotation) * objectToAttach.transform.rotation;
        ...
        public virtual IEnumerator Start()
        {
            ...
            if (!playerInstance)
            {
                MelonLogger.Error("[HPVR Interaction] No player instance found in Hand Start()");
            }

            if (gameObject.layer == 0)
            {
                MelonLogger.Warning("[HPVR Interaction] Hand is on default layer. This puts unnecessary strain on hover checks as it is always true for hand colliders (which are then ignored).");
            }
            else
            {
                hoverLayerMask &= ~(1 << gameObject.layer); //ignore self for hovering
            }
            ...
            //MelonLoader.MelonLogger.Msg( "[HPVR Interaction] Hand - initializing connection routine" );
        ...
        protected virtual void UpdateHovering()
        {
            ...
            //{
            //    float scaledHoverRadius = controllerHoverRadius * Mathf.Abs(SteamVRUtils.GetLossyScale(transform));
            ...
            //{
            //    float scaledHoverRadius = fingerJointHoverRadius * Mathf.Abs(SteamVRUtils.GetLossyScale(transform));
        ...
        protected virtual bool CheckHoveringForTransform(Vector3 hoverPosition, float hoverRadius, ref float closestDistance, ref Interactable closestInteractable, Color debugColor)
        {
            try
            {
                ...
                {
                    MelonLogger.Warning("[HPVR Interaction] This hand is overlapping the max number of colliders: " + ColliderArraySize + ". Some collisions may be missed. Increase ColliderArraySize on Hand.cs");
                }
        ...
        public virtual void FinishInit()
        {
            ...
            float hoverUpdateBegin = handType == InputDeviceRole.LeftHanded ? 0.5f * hoverUpdateInterval : 0.0f;
            ...
            if (attachedObject != null)
            {
                if (currentAttachedObjectInfo.Value.interactable != null)
                {
                    ...
                    if (currentAttachedObjectInfo.Value.interactable.handFollowTransform)
                    {
                        ...
                        //if (pose == null)
                        //{
                        //    Quaternion offset = Quaternion.Inverse(transform.rotation) * currentAttachedObjectInfo.Value.handAttachmentPointTransform.rotation;
                        //    targetHandRotation = currentAttachedObjectInfo.Value.interactable.transform.rotation * Quaternion.Inverse(offset);

                        //    //todo
                        //    //Vector3 worldOffset = transform.position - currentAttachedObjectInfo.Value.handAttachmentPointTransform.position;
                        //    //Quaternion rotationDiff = mainRenderModel.GetHandRotation() * Quaternion.Inverse(transform.rotation);
        ...
        private void HandDebugLog(string msg)
        {
            if (spewDebugText)
            {
                MelonLogger.Msg("[HPVR Interaction] Hand (" + name + "): " + msg);
            }
        }
        ...
        public bool IsGrabbingWithType(GrabTypes type)
        {
            ...
            return type switch
            {
                //GrabTypes.Pinch => (bool)grabPinchAction.GetState(handType),
                //GrabTypes.Grip => (bool)grabGripAction.GetState(handType),
                _ => false,
            };
        }
        ...
        public bool IsGrabbingWithOppositeType(GrabTypes type)
        {
            ...
            return type switch
            {
                //GrabTypes.Pinch => (bool)grabGripAction.GetState(handType),
                //GrabTypes.Grip => (bool)grabPinchAction.GetState(handType),
                _ => false,
            };
        }
        ...
        private void InitController()
        {
            ...
            renderModelPrefab.transform.parent = transform;
